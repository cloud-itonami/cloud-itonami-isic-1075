...
(deftest production-batch-test
  (testing "retrieve an existing batch"
    (let [batch-data {:product-type :meal/cook-chill-poultry :core-cook-temp-c 75.0}
...
(deftest log-batch-test
  (testing "logging a batch marks it as processed"
    (let [st {:batches {}}
          batch-data {:product-type :meal/cook-chill-poultry}
  ...
  (testing "logging preserves batch data"
    (let [st {:batches {}}
          batch-data {:product-type :meal/cook-chill-poultry :core-cook-temp-c 75.0}
          result (store/log-batch st "batch-001" batch-data)]
      (is (= (:product-type (get-in result [:batches "batch-001"])) :meal/cook-chill-poultry))
      (is (= (:core-cook-temp-c (get-in result [:batches "batch-001"])) 75.0)))))

;; ──────────────────────── Shipment Finalization ──────────────────────

(deftest finalize-shipment-test
  (testing "finalizing a batch marks it as finalized"
    (let [st {:batches {"batch-001" {:product-type :meal/cook-chill-poultry}}}
