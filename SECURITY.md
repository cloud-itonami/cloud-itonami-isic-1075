...
If you discover a security issue in cloud-itonami-isic-1075, **do not open a
...
This repository implements the **Governor** for prepared-meals/ready-dish
(cook-chill and cook-freeze) plant operations — a deterministic safety-checking
layer that gates all proposals from the LLM/advisor.

Security-critical concerns:
1. **Hard violations in Governor rules must never be overridable** — a rule
   marked as HARD (e.g., `:already-processed`, `:core-cook-temp-below-minimum`,
   `:cold-storage-temp-out-of-range`, `:foreign-material-detected`,
   `:allergen-label-mismatch`) blocks the proposal unconditionally
